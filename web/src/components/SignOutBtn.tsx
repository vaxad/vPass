...
export default function SignOutBtn() {
  ...
  return !user?(
    ...
    <button onFocus={()=>{setFocus(true)}} onBlur={()=>{setFocus(false)}} className={` flex-nowrap peer overflow-clip pt-0.5 scale-150 aspect-square text-nowrap bg-gradient-to-b from-[#EAEAEA] to-[#848484] border border-slate-50 rounded-full`}>
    ...
    <section className={`absolute bottom-0 right-0  origin-top-right -translate-y-[200%] opacity-[0] scale-50 peer-focus:translate-y-[110%] peer-focus:opacity-[1] peer-focus:scale-100 duration-500 delay-150 transition-all `}>
      <AlertComponent  title='Are you sure?' description={`This will log you out`} onAction={handleSignOut} trigger={
              (
              <div title='Log out' className={` w-fit flex-row flex text-nowrap flex-nowrap gap-2 text-[#B90000] items-center py-2 px-4 bg-black border rounded-lg border-[#515151]`}>
